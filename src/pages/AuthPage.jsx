...
export default function AuthPage({ onLogin }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      let user;
      if (mode === "login") {
        user = signIn({ email: form.email, password: form.password });
      } else {
        ...
        user = signUp({ email: form.email, password: form.password, displayName: form.displayName });
      }
      onLogin(user);
    } catch (err) {
    ...
    <div className="auth-page">
      <div className="auth-card">
        <div className="auth-header">
          ...
          <h1>CrimpClub</h1>
          <p>{mode === "login" ? "Content de te revoir." : "Crée ton logbook."}</p>
        </div>
