...
export default function GymManagerPage({ userId, gyms, onGymsChanged, onBack }) {
      ...
      <div className="page">
          ...
          {gyms.map(g => (
            <div key={g.id} className="gym-row">
              ...
              <div className="gym-row-actions">
                <button className="btn-sm" onClick={() => setEditingGym({ ...g, colors: g.colors.map(c => ({...c})) })}>✏️</button>
        ...
        <div style={{ padding: "16px" }}>
          <button
            ...
            onClick={() => setEditingGym({ id: null, name: "", colors: DEFAULT_COLORS.map(c => ({...c})) })}
          >
            + Ajouter une salle
          </button>
        </div>
  ...
  async function handleSave() {
    ...
    if (editingGym.colors.some(c => !c.name.trim())) { setError("Tous les niveaux doivent avoir un nom."); return; }
    setSaving(true);
    try {
      if (isNew) {
        await addGym(userId, { name: editingGym.name, colors: editingGym.colors });
      } else {
        await updateGym(editingGym.id, { name: editingGym.name, colors: editingGym.colors });
      }
      await onGymsChanged();
      setEditingGym(null);
    } catch (err) {
